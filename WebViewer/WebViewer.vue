	...
	<div ref="primarycontainer" class="primary-container mt-2" v-resize="resize">
        <div :class="{ 'full-screen': fullscreen }" class="viewer-box" id="sizeReference">
            ...
            <div id="stream-controls" class="button-container">
                <v-btn class="full-screen-icon button-maximize" small @click="toggleFullScreen" :title="webViewer.fullScreenText">
                    <v-icon>{{ fullscreen ? 'mdi-window-restore' : 'mdi-window-maximize' }}</v-icon>
                </v-btn>
                
                <v-btn small class="toggle-menu-button-close button-settings" @click="drawer = !drawer" :title="webViewer.settingsText">
					<v-icon>mdi-cog</v-icon>
				</v-btn>

				<v-btn small v-if="paused" @click="startLiveStream" class="toggle-menu-button-close button-play" :title="webViewer.startLiveStream.caption">
					<v-icon>mdi-play</v-icon>
				</v-btn>
				<v-btn small v-if="!paused"  @click="pauseLiveStream" class="toggle-menu-button-close button-play" :title="webViewer.pauseLiveStream.caption">
            ...
            <v-navigation-drawer v-model="drawer" :permanent="drawer" absolute width="350px">
				<v-card outlined>
					...
					<v-card>
						<v-btn @click="startLiveStream" block :title="webViewer.startLiveStream.caption">
							<v-icon class="mr-2">mdi-play</v-icon>
							{{ webViewer.startLiveStream.title }}
						</v-btn>
						<v-btn @click="pauseLiveStream" block :title="webViewer.pauseLiveStream.caption">
							<v-icon class="mr-2">mdi-pause</v-icon>
							{{ webViewer.pauseLiveStream.title }}
						</v-btn>
						<v-btn @click="endLiveStream" block :title="webViewer.endLiveStream.caption">
							<v-icon class="mr-2">mdi-stop</v-icon>
							{{ webViewer.endLiveStream.title }}
						</v-btn>
					</v-card>
				</v-card>
                <v-expansion-panels>
					<v-expansion-panel @click="scrollIntoView">
                        <v-expansion-panel-header :title="webViewer.timeLapse.title">
                            <v-icon class="mr-2">mdi-cog-outline</v-icon>
                            <strong>{{webViewer.settings.title}}</strong>
                    ...
                    <v-expansion-panel @click="scrollIntoView">
                        <v-expansion-panel-header :title="webViewer.overlays.title">
                            <v-icon class="mr-2">mdi-picture-in-picture-top-right</v-icon>
                            <strong>{{webViewer.overlays.caption}}</strong>
                        </v-expansion-panel-header>
                        <v-expansion-panel-content eager>
							<v-row>
								<v-col cols="12" md="4">
									<v-select v-model="selectedPanel" :items="webViewer.overlays.selectedPanelOptions" :label="webViewer.overlays.selectedPanel" hide-details></v-select>
								</v-col>
								<v-col cols="12" md="6">
									<v-select v-model="selectedComponent" :items="webViewer.overlays.selectedComponentOptions" :label="webViewer.overlays.selectedComponent" hide-details></v-select>
								</v-col>
								<v-col cols="12" md="2" style="margin-top: 20px;">
									<v-btn small @click="addComponentsToPanel(selectedPanel, selectedComponent)" class="toggle-menu-button-close" :title="webViewer.overlays.addComponent">
										<v-icon>mdi-plus</v-icon>
									</v-btn>
								</v-col>
							</v-row>
							<v-switch v-for="(child, index) in getComponents('topRight')" v-bind:key="'remove: ' + child.name + index + 'topRight'" @click="removeComponentFromPanel(webViewer.topRight, child)" :label="getTextForComponent('topRight',child)" hide-details class="mt-0 mb-3" :input-value="defaultVSwitch"></v-switch>
							<v-switch v-for="(child, index) in getComponents('topLeft')" v-bind:key="'remove: ' + child.name + index + 'topLeft'" @click="removeComponentFromPanel(webViewer.topLeft, child)" :label="getTextForComponent('topLeft',child)" hide-details class="mt-0 mb-3" :input-value="defaultVSwitch"></v-switch>
							<v-switch v-for="(child, index) in getComponents('bottomRight')" v-bind:key="'remove: ' + child.name + index + 'bottomRight'" @click="removeComponentFromPanel(webViewer.bottomRight, child)" :label="getTextForComponent('bottomRight',child)" hide-details class="mt-0 mb-3" :input-value="defaultVSwitch"></v-switch>
							<v-switch v-for="(child, index) in getComponents('bottomLeft')" v-bind:key="'remove: ' + child.name + index + 'bottomLeft'" @click="removeComponentFromPanel(webViewer.bottomLeft, child)" :label="getTextForComponent('bottomLeft',child)" hide-details class="mt-0 mb-3" :input-value="defaultVSwitch"></v-switch>
                        </v-expansion-panel-content>
                    </v-expansion-panel>
					<v-expansion-panel @click="scrollIntoView">
                        <v-expansion-panel-header :title="webViewer.timeLapse.title">
                            <v-icon class="mr-2">mdi-camera-timer</v-icon>
                            <strong>{{webViewer.timeLapse.title}}</strong>
                        </v-expansion-panel-header>
                        <v-expansion-panel-content eager>
							<v-switch v-model="savedSettings.timeLapseByZOn" :label="webViewer.timeLapseByZimage" hide-details class="mt-0 mb-3"></v-switch>
							<v-switch v-model="savedSettings.timeLapseOn" :label="webViewer.timeLapseByTime" hide-details class="mt-0 mb-3"></v-switch>
							<v-text-field v-model="savedSettings.timeLapseInterval" :label="webViewer.timeLapseByTimeValue" hide-details></v-text-field>
...
export default {
	...
	methods: {
		...
		addComponentsToPanel(panelName, componentName) {
			var panelsObject = [];
			if (panelName == this.webViewer.topLeft) {
				panelsObject = this.panels.topLeft;
			}
			else if (panelName ==  this.webViewer.topRight) {
				panelsObject = this.panels.topRight;
			}
			else if (panelName ==  this.webViewer.bottomLeft) {
				panelsObject = this.panels.bottomLeft;
			}
			else if (panelName ==  this.webViewer.bottomRight) {
				panelsObject = this.panels.bottomRight;
			}
			var componentToAdd = undefined
			if (componentName == this.webViewer.toolPanel) {
				componentToAdd = 'tools-panel';//toolsPanel;
			}
			else if(componentName == this.webViewer.jobEstimationsPanel) {
				componentToAdd = 'job-estimations-panel';//jobEstimationsPanel;
			}
			else if(componentName == this.webViewer.jobControlPanel) {
				componentToAdd = 'job-control-panel';//jobControlPanel;
			}
			else if(componentName == this.webViewer.layerChart) {
				componentToAdd = 'layer-chart';//layerChart;
			}
			else if(componentName == this.webViewer.temperatureChart) {
		...
		getTextForComponent(panel,child) {
			switch(child) {
				case "tools-panel":
					return "Remove Tools Panel (" + this.webViewer[panel] + ")";
				case "job-estimations-panel":
					return "Remove Job Estimation Panel (" + this.webViewer[panel] + ")";
				case "job-control-panel":
					return "Remove Job Control Panel (" + this.webViewer[panel] + ")";
				case "layer-chart":
					return "Remove Layer Chart (" + this.webViewer[panel] + ")";
				case "temperature-chart":
					return "Remove Temperature Chart (" + this.webViewer[panel] + ")";
				default:
					// code block
			}
		},
